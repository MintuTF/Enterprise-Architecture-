...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<Flight> flights =
                    session.createQuery("select e from Flight e join e.origin org join e.airplane ap  where org.country='USA' and ap.capacity>500").list();
          // System.out.println("========");
//            List<Flight> flights =
//                    session.createQuery("select e from Flight e join e.origin org   where org.country='USA' ").list();
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Airline> airlines = session.createQuery("select distinct a from Airline a join a.flights fl where fl.airplane.model='A380'").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f where f.departureTime < '12:00:00' and " +
                    "f.departureDate = '2009-08-07'").list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
